...
export async function writeLedger(entity, identity, source, result, keep = DEFAULT_KEEP) {
    ...
    try {
        await knex(TABLE).insert(row)
            .onConflict(['id', 'schema_hash', 'model', 'prompt_hash']).merge()
        await pruneLedger(knex, entity.id, keep)
        return true
    } catch {
        return false
    }
...
async function pruneLedger(knex, id, keep = DEFAULT_KEEP) {
    if (!Number.isFinite(keep) || keep <= 0) return
    const rows = await knex(TABLE).where({ id })
