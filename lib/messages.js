...
export async function buildMessages({ entity, contentResult, prompt, extraInstruction }) {
    ...
    // Prompt-only — a step asking about what earlier steps already put on
    // meta, with `source: false`. There is no document to attach: the
    // question carries its own inputs, and re-sending a 4 MB PDF to ask
    // about a hundred bytes of JSON already in hand pays for the whole
    // thing twice.
    if (contentResult == null) {
        return [{ role: 'user', content: [{ type: 'text', text: userPrompt }] }]
    }

    // Text path — content is in hand.
    if (typeof contentResult?.content === 'string') {
        if (!contentResult.content.length) {
            return [{ role: 'user', content: [{ type: 'text', text: userPrompt }] }]
        }
        return [{
            role: 'user',
